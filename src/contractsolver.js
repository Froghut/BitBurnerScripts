...
async function sendToServer(ns, type, input, tries) {
	...
	if (!res) {
		ns.print('failed to connect to remote server; retrying in 2s');
		await ns.sleep(2000);
	...
	try {
		...
		ns.print(`error while parsing response ${response}: ${err}; retrying in 2s`);
		await ns.sleep(2000);
		return await sendToServer(ns, type, input, tries + 1);
	}
	...
	if (parsedResp.success) {
		if (parsedResp.type === type && parsedResp.input === input) {
			...
			ns.print(`something went horribly wrong and we got the wrong response back - expected ${type},${input}, got ${parsedResp.type},${parsedResp.input}; retrying in 2s`);
			await ns.sleep(2000);
			return await sendToServer(ns, type, input, tries + 1);
		}
	} else {
		...
		ns.print(`server reported back with error ${parsedResp.error || '[no error]'}; retrying in 2s`);
		await ns.sleep(2000);
		return await sendToServer(ns, type, input, tries + 1);
	}
}
...
async function handleContract(ns, hostname, filename) {
	...
	ns.print(`contract ${filename} at ${hostname} of type ${type} w/ data ${input}`);

	let output = await sendToServer(ns, type, input);
	if (!output) {
		ns.print(`no handler in place for ${type}`);
		return;
	}
	let correct = ns.codingcontract.attempt(output, filename, hostname);
	if (!correct) {
		ns.print(`SERVER GAVE US THE WRONG RESPONSE`);
		throw new Error('I FEEL BETRAYED');
	}
	ns.print(`solved contract ${filename}!`);
}
